...
function selectRandomQuestions(difficulty, count) {
    ...
    if (!pool || pool.length < count) {
        console.error(`Not enough questions! Pool has ${pool?.length || 0}, need ${count}`);
        alert(`Error: Not enough ${difficulty} questions available. Please contact administrator.`);
        return [];
    }
    ...
    // Select exactly the count needed
    const selected = shuffled.slice(0, count);
    
    console.log(`Selected ${selected.length} questions from pool of ${pool.length}`);
    return selected;
}
...
function gradeExam() {
    ...
    // IMPORTANT: Use actual number of questions loaded, not config.questions
    const actualTotal = currentQuestions.length;
    
    console.log(`Grading: ${actualTotal} questions loaded (config said ${config.questions})`);
    ...
    // Use actual total for percentage calculation
    const total = actualTotal;
    const percentage = Math.round((correct / total) * 100);
    const passed = correct >= config.passMarks;
    
    console.log(`Results: ${correct}/${total} = ${percentage}% (pass mark: ${config.passMarks})`);
 ...
 * You'll get this URL after setting up Google Apps Script
 */
const ADMIN_CONFIG = {
    GOOGLE_SHEET_URL: 'YOUR_GOOGLE_SCRIPT_URL_HERE', // Replace with your actual URL
    ENABLE_DATA_COLLECTION: true // Set to false to disable
};

/**
 * Send exam results to Google Sheets for admin dashboard
 */
async function sendResultsToAdmin(result) {
    // Check if data collection is enabled
    if (!ADMIN_CONFIG.ENABLE_DATA_COLLECTION) {
        console.log('Data collection disabled');
        return;
    }

    // Check if URL is configured
    if (ADMIN_CONFIG.GOOGLE_SHEET_URL === 'YOUR_GOOGLE_SCRIPT_URL_HERE') {
    ...
    try {
        const data = {
            ...
            timestamp: new Date().toISOString(),
            ip: currentUser.ip || 'N/A'
        };

        const response = await fetch(ADMIN_CONFIG.GOOGLE_SHEET_URL, {
            method: 'POST',
        ...
        console.log('Results sent to admin dashboard');
    } catch (error) {
...
// Modify the gradeExam function to include admin data collection
const originalGradeExam = gradeExam;
gradeExam = function() {
    // Call original function
    originalGradeExam.call(this);
    
    // Get the result that was just saved
    const result = getStageProgress(currentStage);
    
    // Send to admin dashboard
    if (result) {
        sendResultsToAdmin(result);
    }
};

console.log('Admin data collection module loaded');
